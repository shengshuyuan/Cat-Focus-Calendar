...
- 重做 FoloToy AI Passport 像素风页面为设计稿中的两张主页面：万年历和番茄钟。猫、绿植、书、山景和枝叶等固定插画改用 RGB565 Flash 资源；时间、农历、日期选中态、电量、Wi-Fi 与番茄钟状态仍实时绘制；移除与设计稿不一致的中间农历详情页。1900—2100 离线农历/节气表、BLE Wi-Fi 配网、NTP 校时和 NVS 番茄钟持久化仍可用。
