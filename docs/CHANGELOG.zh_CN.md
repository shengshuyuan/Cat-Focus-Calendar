...
- 修复布偶、加菲专注图仍在睡觉及错位拼接的问题；六张场景改用精确的不透明 RGB565 转换。
- 修复专注或休息截止时按暂停卡在 00:00 的问题；完成事件仍由下一次定时 tick 正常处理。

- 收紧中国日历：顶栏两行、节气放对联井、干支框让出猫咪、宜忌改 13px 短词。
- 万年历底栏箭头改为无柄折线，番茄入口改为红果绿叶。
- 将 BLE Wi-Fi 配网改为 SoftAP 网页门户（`http://192.168.4.1`）。凭据写入 Flash 持久化；加入成功后再走 NTP。
- 重做万年历标题和月历格子：新增 11/13/22 px 字体、格子内农历，并用今天锚点让翻到他月停在 1 号、回到本月仍选中今天。
- 锁定中国日历图层顺序、农历/节气顶栏居中，以及宜忌各两行。番茄钟同一皮肤共用一种纸色，专注/休息只切换猫咪场景图。
- 布偶和加菲（异短橘猫）番茄钟房间统一成休息图那套暖光；开倒计时只换猫咪姿势，墙色不再跳变。
- 睡觉的番茄钟猫咪改为清楚的眯眼；专注坐姿保持圆眼睁开、卖萌表情。
- 更新 README 效果图顺序：第一张为已连接 Wi-Fi 的月历，第三张为 24:59 专注倒计时。两张均基于源码渲染，并非真机截屏。

- 新增十分钟闲置背光策略：只将背光降为 0，不丢失当前页面或番茄钟状态；首次符合条件的按键只唤醒屏幕，下一次按键才操作 UI。
- 由于无 PSRAM 的 Wi-Fi 构建禁用了整屏串口截屏，仓库继续使用页面渲染效果图作为视觉参考。

- 新增中国撕页日历页面：在万年历长按 `UP` 进入，`UP`、`DOWN` 按天前后切换，`OK` 返回万年历。页面复用已有离线日期数据，并明确轻量的专注建议不等同于权威传统黄历。

- 新增清醒站立三花猫的番茄钟插画。仅在专注计时进行中替换睡猫场景；待开始、暂停和休息状态继续使用休息插画。

- 重做 FoloToy AI Passport 像素风页面为设计稿中的两张主页面：万年历和番茄钟。猫、绿植、书、山景和枝叶等固定插画改用 RGB565 Flash 资源；时间、农历、日期选中态、电量、Wi-Fi 与番茄钟状态仍实时绘制；移除与设计稿不一致的中间农历详情页。1900—2100 离线农历/节气表、SoftAP Wi-Fi 配网、NTP 校时和 NVS 番茄钟持久化仍可用。
